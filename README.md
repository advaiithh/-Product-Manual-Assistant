...
## ⚙️ Architecture
PDF Manual → PyMuPDF Text Extraction → Noise Removal + Context Compression → Semantic Chunking → Embeddings → FAISS Vector Index → Top-K Retrieval → LLM (RAG Prompt) → Answer with Page Citations → Streamlit Chat UI
...
## 🚀 Installation
https://github.com/advaiithh/-Product-Manual-Assistant
cd ManualQ
...
## 🔍 Retrieval + Citation
1) Embed query → 2) Top-K from FAISS → 3) Pass chunks with page metadata → 4) LLM answers with citations.
